	...
	<div class="d-flex justify-content-center">
        ...
        <form action="{{ url('register') }}" method="post" class="login-form text-left @if (count($errors) > 0)vivify shake @endif" name="form" id="signup_form">
          ...
          @if($settings->facebook_login == 'on')
              <div class="mb-2">
                <a href="{{url('oauth/facebook')}}" class="btn btn-block btn-lg fb-button no-hover" style="color:white;"><i class="fab fa-facebook mr-2"></i> {{trans('misc.sign_up_with')}} Facebook</a>
              </div>
            @endif

            @if($settings->google_login == 'on')
                <div class="mb-2">
                  <a href="{{url('oauth/google')}}" class="btn btn-block btn-lg google-button no-hover"><img src="{{url('public/img/google.svg')}}" width="18" height="18" style="vertical-align: text-bottom;" class="mr-2"> {{trans('misc.sign_up_with')}} Google</a>
                </div>
              @endif
          ...
          <div class="custom-control custom-checkbox my-1">
           <input type="checkbox" required @if( old('agree_gdpr') ) checked="checked" @endif class="custom-control-input" name="agree_gdpr" id="customControlInline" value="1">
           <label class="custom-control-label" for="customControlInline">{{ trans('admin.i_agree_gdpr') }}
